...
class Stencil:
    ...
    def __init__(self):
        ...
        self.label2 = Label(self.frame3, text=f'Save Image as Stencil: {"   "*19}', font='Arial 10 bold',
                            fg='#a6a6ff', bg='#09446e')
    ...
    def convert_image(self):
        ...
        if convert:
            try:
                ...
                # dim = (width, height)
                # imageTmp = cv2.resize(image, dim, interpolation=cv2.INTER_AREA)
                # cv2.imshow("Preview", imageTmp)
                # cv2.waitKey(0)
                # # ]
